...
rule final_contacts_bwa_mem2_index_prim:
    ...
    output:
        "results/Scaffolding/YAHS_Scaffolding/{sample}.yahs_scaffolds_final.fa.bwt.2bit.64"
    log:
...
rule final_contacts_bwa_mem2_mapping_prim_r1:
    input:
        fasta="results/Scaffolding/YAHS_Scaffolding/{sample}.yahs_scaffolds_final.fa",
        hic_r1="results/Trimming_QC/HiC/{sample}_R1.trimmed_paired.fastq.gz",
        index="results/Scaffolding/YAHS_Scaffolding/{sample}.yahs_scaffolds_final.fa.bwt.2bit.64",
    output:
        "results/Scaffolding/Final_Contacts/{sample}.R1.bam"
    log:
        "logs/{sample}.final_contacts_bwa_mem2_mapping_prim_r1.txt"
    benchmark:
        "benchmarks/{sample}.final_contacts_bwa_mem2_mapping_prim_r1.txt"
    params:
        tmpdir="results/Scaffolding/Final_Contacts/tmp1"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule final_contacts_bellerophon_merge_prim:
    input:
        bam_r1="results/Scaffolding/Final_Contacts/{sample}.R1.bam",
        bam_r2="results/Scaffolding/Final_Contacts/{sample}.R2.bam",
    output:
        "results/Scaffolding/Final_Contacts/{sample}.merged.bam"
    params:
        quality= config['bellerophon'],
    log:
...
rule final_contacts_samtools_flagstats_prim:
    input:
        merged_bam="results/Scaffolding/Final_Contacts/{sample}.merged.bam",
    output:
        flagstats="results/Scaffolding/Final_Contacts/{sample}.merged.bam.flagstats",
        hic_stats="results/Scaffolding/Final_Contacts/{sample}.merged.bam.hic_stats"
    log:
...
rule final_contacts_pretext_map_snapshot_prim:
    input:
        merged_bam="results/Scaffolding/Final_Contacts/{sample}.merged.bam",
    output:
        "results/Scaffolding/Final_Contacts/{sample}.merged.bam.pretext"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
    ...
    shell:
        """
        samtools view -h {input.merged_bam} | PretextMap -o {output} {params.pretextmap} >> {log} 2>&1 && \
        PretextSnapshot -m {output} {params.snapshot} -o results/Scaffolding/Final_Contacts/ --prefix {wildcards.sample}.final_contacts_prim >> {log} 2>&1
        """
...
rule final_contacts_samtools_flagstats_bam_r1_prim:
    input:
        bam_r1="results/Scaffolding/Final_Contacts/{sample}.R1.bam",
    output:
        flagstats_r1="results/Scaffolding/Final_Contacts/{sample}.R1.bam.flagstats"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
...
rule final_contacts_samtools_flagstats_bam_r2_prim:
    input:
        bam_r2="results/Scaffolding/Final_Contacts/{sample}.R2.bam",
    output:
        flagstats_r2="results/Scaffolding/Final_Contacts/{sample}.R2.bam.flagstats"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
