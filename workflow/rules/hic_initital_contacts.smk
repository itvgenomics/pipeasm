...
rule initial_contacts_bwa_mem2_index_prim:
    ...
    shell:
        """
        mkdir -p results/Scaffolding/Initial_Contacts && \
        bwa-mem2.avx index {input} >> {log} 2>&1
        """
...
rule initial_contacts_bellerophon_merge_prim:
    input:
        bam_r1="results/Scaffolding/Initial_Contacts/{sample}.R1.bam",
        bam_r2="results/Scaffolding/Initial_Contacts/{sample}.R2.bam",
    output:
        "results/Scaffolding/Initial_Contacts/{sample}.merged.bam"
    params:
        quality= config['bellerophon'],
    log:
...
rule initial_contacts_samtools_flagstats_prim:
    ...
    output:
        flagstats="results/Scaffolding/Initial_Contacts/{sample}.merged.bam.flagstats",
        hic_stats="results/Scaffolding/Initial_Contacts/{sample}.merged.bam.hic_stats"
    log:
...
rule initial_contacts_pretext_map_snapshot_prim:
    input:
        merged_bam="results/Scaffolding/Initial_Contacts/{sample}.merged.bam",
    output:
        "results/Scaffolding/Initial_Contacts/{sample}.merged.bam.pretext"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
    ...
    shell:
        """
        samtools view -h {input.merged_bam} | PretextMap -o {output} {params.pretextmap} >> {log} 2>&1 && \
        PretextSnapshot -m {output} {params.snapshot} -o results/Scaffolding/Initial_Contacts/ --prefix {wildcards.sample}_prim >> {log} 2>&1
        """
...
rule initial_contacts_samtools_flagstats_bam_r1_prim:
    input:
        bam_r1="results/Scaffolding/Initial_Contacts/{sample}.R1.bam",
    output:
        flagstats_r1="results/Scaffolding/Initial_Contacts/{sample}.R1.bam.flagstats"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
...
rule initial_contacts_samtools_flagstats_bam_r2_prim:
    input:
        bam_r2="results/Scaffolding/Initial_Contacts/{sample}.R2.bam",
    output:
        flagstats_r2="results/Scaffolding/Initial_Contacts/{sample}.R2.bam.flagstats"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
