if config["gxdb"]:
    ...
    index_out_prim = "results/Decontamination/Contaminants/Solo_Asm_Primary/{sample}.p_ctg.clean.fasta.bwt.2bit.64"
    index_out_alt = "results/Decontamination/Contaminants/Solo_Asm_Alt/{sample}.a_ctg.clean.fasta.bwt.2bit.64"
else:
    ...
    index_out_prim = "results/Assembly/Contigging/Solo_Asm/{sample}.p_ctg.fa.bwt.2bit.64"
    index_out_alt = "results/Assembly/Contigging/Solo_Asm/{sample}.a_ctg.fa.bwt.2bit.64"
...
rule initial_contacts_bwa_mem2_index_prim:
    input:
        lambda wildcards: "results/Decontamination/Contaminants/Solo_Asm_Primary/{sample}.p_ctg.clean.fasta" if config["gxdb"] else "results/Assembly/Contigging/Solo_Asm/{sample}.p_ctg.fa"
    output: index_out_prim
    log:
        "logs/{sample}.initial_contacts_bwa_mem2_index_prim.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_bwa_mem2_index_prim.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
        mkdir -p results/Scaffolding/Initial_Contacts/Primary && \
        bwa-mem2.avx index {input} >> {log} 2>&1
        """

rule initial_contacts_bwa_mem2_index_alt:
    input:
        lambda wildcards: "results/Decontamination/Contaminants/Solo_Asm_Alt/{sample}.a_ctg.clean.fasta" if config["gxdb"] else "results/Assembly/Contigging/Solo_Asm/{sample}.a_ctg.fa"
    output: index_out_alt
    log:
        "logs/{sample}.initial_contacts_bwa_mem2_index_alt.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_bwa_mem2_index_alt.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
        mkdir -p results/Scaffolding/Initial_Contacts/Alternative && \
        bwa-mem2.avx index {input} >> {log} 2>&1
        """
...
rule initial_contacts_bwa_mem2_mapping_prim_r1:
    input:
        ...
        hic_r1="results/Trimming_QC/HiC/{sample}_R1.trimmed_paired.fastq.gz",
        index=lambda wildcards: "results/Decontamination/Contaminants/Solo_Asm_Primary/{sample}.p_ctg.clean.fasta.bwt.2bit.64" if config["gxdb"] else "results/Assembly/Contigging/Solo_Asm/{sample}.p_ctg.fa.bwt.2bit.64"
    output:
        "results/Scaffolding/Initial_Contacts/Primary/{sample}.R1.bam"
    log:
        "logs/{sample}.initial_contacts_bwa_mem2_mapping_prim_r1.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_bwa_mem2_mapping_prim_r1.txt"
    params:
        tmpdir="results/Scaffolding/Initial_Contacts/Primary/tmp1"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule initial_contacts_bwa_mem2_mapping_prim_r2:
    input:
        fasta=lambda wildcards: "results/Decontamination/Contaminants/Solo_Asm_Primary/{sample}.p_ctg.clean.fasta" if config["gxdb"] else "results/Assembly/Contigging/Solo_Asm/{sample}.p_ctg.fa",
        hic_r2="results/Trimming_QC/HiC/{sample}_R2.trimmed_paired.fastq.gz",
        index=lambda wildcards: "results/Decontamination/Contaminants/Solo_Asm_Primary/{sample}.p_ctg.clean.fasta.bwt.2bit.64" if config["gxdb"] else "results/Assembly/Contigging/Solo_Asm/{sample}.p_ctg.fa.bwt.2bit.64"
    output:
        "results/Scaffolding/Initial_Contacts/Primary/{sample}.R2.bam"
    log:
        "logs/{sample}.initial_contacts_bwa_mem2_mapping_prim_r2.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_bwa_mem2_mapping_prim_r2.txt"
    params:
        tmpdir="results/Scaffolding/Initial_Contacts/Primary/tmp2"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule initial_contacts_bwa_mem2_mapping_alt_r1:
    input:
        ...
        hic_r1="results/Trimming_QC/HiC/{sample}_R1.trimmed_paired.fastq.gz",
        index=lambda wildcards: "results/Decontamination/Contaminants/Solo_Asm_Alt/{sample}.a_ctg.clean.fasta.bwt.2bit.64" if config["gxdb"] else "results/Assembly/Contigging/Solo_Asm/{sample}.a_ctg.fa.bwt.2bit.64"
    output:
        "results/Scaffolding/Initial_Contacts/Alternative/{sample}.R1.bam"
    log:
        "logs/{sample}.initial_contacts_bwa_mem2_mapping_alt_r1.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_bwa_mem2_mapping_alt_r1.txt"
    params:
        tmpdir="results/Scaffolding/Initial_Contacts/Alternative/tmp1"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule initial_contacts_bwa_mem2_mapping_alt_r2:
    input:
        fasta=lambda wildcards: "results/Decontamination/Contaminants/Solo_Asm_Alt/{sample}.a_ctg.clean.fasta" if config["gxdb"] else "results/Assembly/Contigging/Solo_Asm/{sample}.a_ctg.fa",
        hic_r2="results/Trimming_QC/HiC/{sample}_R2.trimmed_paired.fastq.gz",
        index=lambda wildcards: "results/Decontamination/Contaminants/Solo_Asm_Alt/{sample}.a_ctg.clean.fasta.bwt.2bit.64" if config["gxdb"] else "results/Assembly/Contigging/Solo_Asm/{sample}.a_ctg.fa.bwt.2bit.64"
    output:
        "results/Scaffolding/Initial_Contacts/Alternative/{sample}.R2.bam"
    log:
        "logs/{sample}.initial_contacts_bwa_mem2_mapping_alt_r2.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_bwa_mem2_mapping_alt_r2.txt"
    params:
        tmpdir="results/Scaffolding/Initial_Contacts/Alternative/tmp2"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule initial_contacts_bellerophon_merge_prim:
    input:
        bam_r1="results/Scaffolding/Initial_Contacts/Primary/{sample}.R1.bam",
        bam_r2="results/Scaffolding/Initial_Contacts/Primary/{sample}.R2.bam",
    output:
        "results/Scaffolding/Initial_Contacts/Primary/{sample}.merged.bam"
    params:
        quality= config['bellerophon'],
    log:
        "logs/{sample}.initial_contacts_bellerophon_merge_prim.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_bellerophon_merge_prim.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule initial_contacts_bellerophon_merge_alt:
    input:
        bam_r1="results/Scaffolding/Initial_Contacts/Alternative/{sample}.R1.bam",
        bam_r2="results/Scaffolding/Initial_Contacts/Alternative/{sample}.R2.bam",
    output:
        "results/Scaffolding/Initial_Contacts/Alternative/{sample}.merged.bam"
    params:
        quality= config['bellerophon'],
    log:
        "logs/{sample}.initial_contacts_bellerophon_merge_alt.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_bellerophon_merge_alt.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule initial_contacts_samtools_flagstats_prim:
    input:
        merged_bam="results/Scaffolding/Initial_Contacts/Primary/{sample}.merged.bam",
    output:
        flagstats="results/Scaffolding/Initial_Contacts/Primary/{sample}.merged.bam.flagstats",
        hic_stats="results/Scaffolding/Initial_Contacts/Primary/{sample}.merged.bam.hic_stats"
    log:
        "logs/{sample}.initial_contacts_samtools_flagstats_prim.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_samtools_flagstats_prim.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule initial_contacts_samtools_flagstats_alt:
    input:
        merged_bam="results/Scaffolding/Initial_Contacts/Alternative/{sample}.merged.bam",
    output:
        flagstats="results/Scaffolding/Initial_Contacts/Alternative/{sample}.merged.bam.flagstats",
        hic_stats="results/Scaffolding/Initial_Contacts/Alternative/{sample}.merged.bam.hic_stats"
    log:
        "logs/{sample}.initial_contacts_samtools_flagstats_alt.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_samtools_flagstats_alt.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule initial_contacts_pretext_map_snapshot_prim:
    input:
        merged_bam="results/Scaffolding/Initial_Contacts/Primary/{sample}.merged.bam",
    output:
        "results/Scaffolding/Initial_Contacts/Primary/{sample}.merged.bam.pretext"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
        "logs/{sample}.initial_contacts_pretext_map_snapshot_prim.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_pretext_map_snapshot_prim.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
        samtools view -h {input.merged_bam} | PretextMap -o {output} {params.pretextmap} >> {log} 2>&1 && \
        PretextSnapshot -m {output} {params.snapshot} -o results/Scaffolding/Initial_Contacts/Primary --prefix {wildcards.sample}_prim >> {log} 2>&1
        """

rule initial_contacts_pretext_map_snapshot_alt:
    input:
        merged_bam="results/Scaffolding/Initial_Contacts/Alternative/{sample}.merged.bam",
    output:
        "results/Scaffolding/Initial_Contacts/Alternative/{sample}.merged.bam.pretext"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
        "logs/{sample}.initial_contacts_pretext_map_snapshot_alt.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_pretext_map_snapshot_alt.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
        samtools view -h {input.merged_bam} | PretextMap -o {output} {params.pretextmap} >> {log} 2>&1 && \
        PretextSnapshot -m {output} {params.snapshot} -o results/Scaffolding/Initial_Contacts/Alternative --prefix {wildcards.sample}_alt >> {log} 2>&1
        """
...
rule initial_contacts_samtools_flagstats_bam_r1_prim:
    input:
        bam_r1="results/Scaffolding/Initial_Contacts/Primary/{sample}.R1.bam",
    output:
        flagstats_r1="results/Scaffolding/Initial_Contacts/Primary/{sample}.R1.bam.flagstats"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
        "logs/{sample}.initial_contacts_samtools_flagstats_bam_r1_prim.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_samtools_flagstats_bam_r1_prim.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule initial_contacts_samtools_flagstats_bam_r2_prim:
    input:
        bam_r2="results/Scaffolding/Initial_Contacts/Primary/{sample}.R2.bam",
    output:
        flagstats_r2="results/Scaffolding/Initial_Contacts/Primary/{sample}.R2.bam.flagstats"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
        "logs/{sample}.initial_contacts_samtools_flagstats_bam_r2_prim.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_samtools_flagstats_bam_r2_prim.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule initial_contacts_samtools_flagstats_bam_r1_alt:
    input:
        bam_r1="results/Scaffolding/Initial_Contacts/Alternative/{sample}.R1.bam",
    output:
        flagstats_r1="results/Scaffolding/Initial_Contacts/Alternative/{sample}.R1.bam.flagstats"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
        "logs/{sample}.initial_contacts_samtools_flagstats_bam_r1_alt.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_samtools_flagstats_bam_r1_alt.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
...
rule initial_contacts_samtools_flagstats_bam_r2_alt:
    input:
        bam_r2="results/Scaffolding/Initial_Contacts/Alternative/{sample}.R2.bam",
    output:
        flagstats_r2="results/Scaffolding/Initial_Contacts/Alternative/{sample}.R2.bam.flagstats"
    params:
        pretextmap= config['pretext']["map"],
        snapshot=  config['pretext']["snapshot"]
    log:
        "logs/{sample}.initial_contacts_samtools_flagstats_bam_r2_alt.txt"
    benchmark:
        "benchmarks/{sample}.initial_contacts_samtools_flagstats_bam_r2_alt.txt"
    singularity:
       f"{config["sif_dir"]}/hic_mapping.sif"
    shell:
        """
